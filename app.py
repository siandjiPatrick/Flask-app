...
"""
with app.app_context():
    all_designation = [des.name for des in Designation.query.all()]                                                                                                                                                                                                                     
    print(f"designations:{all_designation}")
...
@app.route('/manage_revenues/', defaults={ 'designations': '' }, methods=['GET', 'POST'])
@app.route('/manage_revenues/<string:designations>', methods=['GET', 'POST'])
@login_required
def manage_revenues(designations):
    ...
    if request.method == 'POST':
        ...
        new_revenues = Revenues(
                            designation = Designation.query.filter_by(name=designations).first(),
                            description = description,
                            date = date,
                            montant = montant,
                            user = User.query.filter_by(lastname=provenance).first()                    
                        )
...
@app.route('/manage_designations',defaults={ 'ref': '' }, methods=['GET', 'POST'])
@app.route('/manage_designations/<string:ref>', methods=['GET', 'POST'])
@login_required
def manage_designations(ref):
    ...
    if request.method == 'POST':
        ...
        if ref != "":
            ma_reference=ref
            return redirect(url_for('choose_designation', ref=ma_ref))
...
#@app.route('/choisir_une_designation',defaults={ 'ref': '' }, methods=['GET', 'POST'])
@app.route("/choisir_une_designation/<string:ref>", methods = ["POST", "GET"])
@login_required
def choose_designation(ref):

    choix_designation = request.form.get("designation")
    all_designation = Designation.query.all()
    if request.method == "POST":
        

        #print(choix_designation)
        choix_designation = request.form.get("designation")
        print(ref)
        print(choix_designation)

        if choix_designation == "":
            return redirect(url_for("manage_designations", ref=ref))
        
        elif choix_designation != "" and (ref == "ref_revenu"):
            return  redirect(url_for("manage_revenues",designations=choix_designation))
        
        elif choix_designation != "" and (ref == "ref_depense"):
            return  redirect(url_for("manage_depenses",designations=choix_designation))
        
        elif choix_designation != "" and (ref == "ref_epargne"):
            return  redirect(url_for("manage_epargnes",designations=choix_designation))

    return render_template('choose_designation.html', user=current_user, designations=all_designation)
